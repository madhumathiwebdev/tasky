...
<div class="card-header gap-2 d-flex justify-content-end">
 <button class="btn btn-outline-info "  name=${taskData.id} onclick ="editCard.apply(this,arguments)"1>
...
<div class="card-footer">
 <button type ="button" class=" btn btn-outline-primary" name=${taskData.id}  data-bs-toggle="modal"
 data-bs-target="#showTask" name=  ${taskData.image}
...
localStorage.setItem("taskyAM",JSON.stringify({cards:globalTaskData}));
...
const loadExistingCards =() =>{
    ...
    // check local storage
   const  getData =localStorage.getItem("taskyAM");
   ...
   globalTaskData.map((taskData)=>{
    //generate html code for those datas
    const newCard =generateHTML(taskData);
